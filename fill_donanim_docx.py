# -*- coding: utf-8 -*-
"""
donanim_defteri_sozel.md icerigini bos sablondan uretilen
donanim_staj.docx dosyasina gun gun (Gun 1..Gun 20) yerlestirir.
"""
import re
import copy
import datetime
...
MD_PATH = "donanim_defteri_sozel.md"
DOCX_IN = "yazilim_staj_yedek.docx"  # bozulmamis orijinal bos sablon
DOCX_OUT = "donanim_staj.docx"
...
def set_paragraph_text(paragraph, text, size=12):
    """Paragraftaki (parcalanmis olabilecek) tum run'lari temizleyip metni ilk run'a yazar."""
...
def fill_cover_tables(doc, start_str, end_str, total_days):
    ...
    YARIYIL = "2025 / 2026"
    KURUM = "Mezitli Belediyesi"
    STAJ_KONUSU = "Donanım Stajı"
    ...
    # "Staj Yaptigi Kurum" hucresi 5 alt paragraftan olusuyor, deger 2. paragrafa (index 1) yazilir
...
def fix_section_break_to_nextpage(doc):
    """Ozet tablo ile Gun 1 arasindaki bolum (section) gecisi 'continuous' oldugu
    icin Gun 1, ozet tablonun tasan son satiriyla ayni sayfada kalip hem gereksiz
    bir tablo parcasi hem de imza altligi olmayan bir sayfa olusturuyordu. Bu
    gecisi 'nextPage' yaparak Gun 1'in kendi sayfasinda, dogru altbilgiyle
    (Is yeri Amirinin Unvani / Imza ve Kase) baslamasini sagliyoruz."""
    ...
    # Bolum artik Gun 1'in kendi sayfasinda basladigi icin sayfa numaralandirmasi
    # (her gunun "Sayfa No" PAGE alaninin dayandigi baslangic) 0 degil 1'den
    # baslamali; boylece Gun 1 sayfasi "1", Gun 2 "2" seklinde devam eder.
...
def main():
    ...
    for i, day in enumerate(DAYS):
        ...
        # Sablonda gercek gun sayisindan (len(DAYS)) fazla bos "Gun" tablosu olabilir
        # (bu sablon 30 gunluk bir staj icin hazirlanmis, sadece ilk len(DAYS) tanesi kullanilir).
    ...
    # Sablonda gercek gun sayisindan fazla kalan bos "Gun" tablolarini (ve aralarindaki
    # bos paragraflari) belge sonundan (sectPr haric) tamamen kaldir.
    if len(doc.tables) > 4 + len(DAYS):
        first_extra_table = doc.tables[4 + len(DAYS)]
        children = list(body.iterchildren())
        extra_start_idx = children.index(first_extra_table._tbl)
        # son eleman sectPr'dir, onu koru; digerlerinin hepsini sil
