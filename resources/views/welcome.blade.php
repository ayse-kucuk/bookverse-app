...
<head>
    @include('partials.head', [
        'title' => 'Bookverse — ' . __('ui.nav.explore'),
        'description' => __('ui.seo.explore_description'),
...
<body class="bv-mesh min-h-screen antialiased selection:bg-[#e8dfd2]">
    ...
    <header class="bv-page bv-animate-up pb-6 pt-8 text-center sm:pb-8 sm:pt-16">
        <span class="bv-badge mb-4">{{ __('ui.explore.badge') }}</span>
        <h1 class="bv-hero-title">
            <span class="bv-gradient-text">{{ __('ui.explore.title') }}</span>
        </h1>
        <p class="bv-animate-up-delay-1 mx-auto mt-4 max-w-md text-sm leading-relaxed text-[#6b6560]">{{ __('ui.explore.subtitle') }}</p>
    </header>
    ...
    <main class="bv-page pb-24">
        ...
        <form method="GET" action="{{ route('explore') }}" class="bv-card bv-animate-up-delay-1 mb-6 flex flex-col gap-3 rounded-2xl p-4 sm:flex-row sm:items-end">
            <div class="min-w-0 flex-1">
                <label for="explore-q" class="mb-1 block text-[10px] font-bold uppercase tracking-wider text-slate-400">{{ __('ui.explore.search') }}</label>
                <input id="explore-q" type="search" name="q" value="{{ $searchQuery ?? '' }}" placeholder="{{ __('ui.explore.search_placeholder') }}" class="bv-input w-full rounded-xl border border-slate-200 px-3 py-2 text-sm">
            </div>
            <div class="sm:w-40">
                <label for="explore-category" class="mb-1 block text-[10px] font-bold uppercase tracking-wider text-slate-400">{{ __('ui.explore.category') }}</label>
            ...
            <div class="sm:w-36">
                <label for="explore-sort" class="mb-1 block text-[10px] font-bold uppercase tracking-wider text-slate-400">{{ __('ui.explore.sort') }}</label>
                <select id="explore-sort" name="sort" class="bv-input w-full rounded-xl border border-slate-200 px-3 py-2 text-sm">
                    <option value="latest" @selected(($currentSort ?? 'latest') === 'latest')>{{ __('ui.explore.sort_latest') }}</option>
                    <option value="title" @selected(($currentSort ?? '') === 'title')>{{ __('ui.explore.sort_title') }}</option>
                    <option value="rating" @selected(($currentSort ?? '') === 'rating')>{{ __('ui.explore.sort_rating') }}</option>
                </select>
            </div>
            <button type="submit" class="bv-btn rounded-xl px-5 py-2 text-sm font-bold text-white">{{ __('ui.explore.filter') }}</button>
        </form>
