...
<head>
    @include('partials.head', ['title' => __('ui.profile.page_title')])
</head>
    ...
    <main class="bv-page space-y-5 py-6 sm:py-10">
        ...
        <section class="bv-card bv-animate-up flex h-full w-full flex-col items-center gap-4 p-4 text-center sm:flex-row sm:items-center sm:gap-6 sm:p-6 sm:text-left">
            @if($user->profile_photo_path)
                <button type="button" onclick="openPhotoLightbox()" class="bv-photo-trigger flex h-24 w-24 shrink-0 items-center justify-center overflow-hidden rounded-full border-2 border-[#e8e4de] sm:h-32 sm:w-32" aria-label="{{ __('ui.profile.photo_enlarge') }}">
            ...
            <div class="min-w-0 flex-1">
                ...
                <div class="mt-3">
                    <a href="{{ route('account.settings') }}" class="text-[10px] font-bold uppercase tracking-wider text-[#9a948d] transition hover:text-bv-accent">{{ __('ui.profile.settings') }}</a>
                </div>
            </div>
        </section>
        ...
        <section class="bv-animate-up-delay-2 w-full space-y-4">
            <h2 class="text-[10px] font-bold uppercase tracking-[0.2em] text-[#9a948d]">{{ __('ui.profile.my_posts') }}</h2>
            <div class="bv-stagger space-y-4">
                @forelse($posts as $post)
                    @include('partials.post-card', ['post' => $post])
                @empty
                    <div class="bv-card p-8 text-center">
                        <p class="text-sm text-[#9a948d]">{{ __('ui.profile.no_posts') }}</p>
                        <a href="{{ route('home') }}" class="mt-2 inline-block text-xs font-bold uppercase tracking-wider text-bv-accent hover:underline">{{ __('ui.profile.go_feed') }}</a>
                    </div>
                @endforelse
            </div>
            <div>{{ $posts->links() }}</div>
